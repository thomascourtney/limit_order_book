...
void LimitOrderBook::matchOrders() {
    for (auto& buy_order : buy_orders) {
        ...
        if (buy_price >= sell_price) {
            ...
            // Calculate and print live PnL here
            totalPnL = calculatePnL();
            printLivePnL();

            // Add a delay to see the process
            std::this_thread::sleep_for(std::chrono::milliseconds(100)); //200 milliseconds delay
        }
    }
}

int LimitOrderBook::calculatePnL() {
    int pnl = 0;

    for (auto& buy_order : buy_orders) {
        for (const Order& order : buy_order.second) {
            pnl -= order.price * order.quantity;
        }
    }

    for (auto& sell_order : sell_orders) {
        for (const Order& order : sell_order.second) {
            pnl += order.price * order.quantity;
        }
    }
...
void LimitOrderBook::printLivePnL() {
    cout << "Live PnL: " << totalPnL << endl;
}
...
int main() {
    ...
    // Ensure PnL starts at 0
    orderBook.printLivePnL();
